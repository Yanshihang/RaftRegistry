...
// 明确区分 char[N] 和 char* 允许 lexical_cast 函数准确处理固定大小数组的情况，并利用数组大小的编译时信息。这种区分可以提高类型安全性，确保在编译时就能捕获到一些潜在的错误，同时也允许对固定大小数组的特定优化。
 ...
 */
template <typename To, typename From>
std::enable_if<!std::is_same<To, From>::value, To>::type lexical_cast(const From& from) {
